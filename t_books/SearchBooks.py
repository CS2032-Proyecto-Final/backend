...
def lambda_handler(event, context):
    # Obtener parámetros de entrada obligatorios
    ...
    title = event['query'].get('title', None)
    author_name = event['query'].get('author_name', None)
    author_lastname = event['query'].get('author_lastname', None)
    isbn = event['query'].get('isbn', None)
    ...
    # Configurar la expresión de consulta base
    query_params = {
    ...
    if title:
        query_params['FilterExpression'] = Attr('title').begins_with(title)
    elif author_name:
        query_params['FilterExpression'] = Attr('author_name').begins_with(author_name)
    elif author_lastname:
        query_params['FilterExpression'] = Attr('author_lastname').begins_with(author_lastname)
    elif isbn:
        query_params['KeyConditionExpression'] = Key('tenant_id').eq(tenant_id) & Key('isbn').eq(isbn)
